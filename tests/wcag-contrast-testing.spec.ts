...
function isLargeText(fontSize: string, fontWeight: string): boolean {
  ...
  return sizeInPx >= 18 || (sizeInPx >= 14 && (fontWeight === 'bold' || parseInt(fontWeight) >= 700));
}
    ...
    for (const element of elementsToTest) {
      try {
        ...
        if (colors && colors.color && colors.backgroundColor) {
          ...
          const minRatio = isLarge ? WCAG_RATIOS.LARGE_TEXT : WCAG_RATIOS.NORMAL_TEXT;
          
          console.log(`${element.description}: Ratio ${ratio.toFixed(2)} (Required: ${minRatio})`);
    ...
    for (const element of formElements) {
      try {
        ...
        if (colors && colors.color && colors.backgroundColor) {
          ...
          console.log(`Form ${element}: Ratio ${ratio.toFixed(2)} (Required: ${minRatio})`);
          expect(ratio).toBeGreaterThanOrEqual(minRatio);
        }
      } catch (error) {
    ...
    for (const element of darkModeElements) {
      try {
        ...
        if (colors && colors.color && colors.backgroundColor) {
          ...
          console.log(`Dark mode ${element}: Ratio ${ratio.toFixed(2)} (Required: ${minRatio})`);
          expect(ratio).toBeGreaterThanOrEqual(minRatio);
        }
      } catch (error) {
  ...
  test('Color-only Information Detection', async ({ page }) => {
    ...
    const colorOnlyPatterns = await page.evaluate(() => {
      ...
      allElements.forEach(el => {
        ...
        if (computed.color.includes('rgb(220') || computed.color.includes('red')) {
          const hasIcon = el.querySelector('svg, .icon, [class*="icon"]');
          const hasText = el.textContent?.includes('erro') || el.textContent?.includes('inválido');
    ...
    if (colorOnlyPatterns.length > 0) {
      console.warn('⚠️  Potential color-only information detected:', colorOnlyPatterns);
    }
  });
    ...
    for (let i = 0; i < Math.min(interactiveElements.length, 5); i++) {
      ...
      states.forEach(state => {
        if (state.colors.color && state.colors.backgroundColor) {
          const ratio = calculateContrastRatio(state.colors.color, state.colors.backgroundColor);
          console.log(`Interactive element ${i} (${state.name}): Ratio ${ratio.toFixed(2)}`);
          
          if (ratio < WCAG_RATIOS.UI_COMPONENTS) {
            console.warn(`⚠️  Low contrast in ${state.name} state: ${ratio.toFixed(2)}`);
          }
        }
      });

      // Check focus indicators
      if (focusColors.outline === 'none' && !focusColors.boxShadow.includes('rgb')) {
  ...
  test('Comprehensive WCAG Summary', async ({ page }) => {
    ...
    for (const pagePath of pages) {
      try {
        ...
        const pageElements = ['h1', 'h2', 'h3', 'p', 'button', 'a', 'input', 'label'];
        ...
        for (const selector of pageElements) {
          try {
            ...
            for (let i = 0; i < Math.min(elements.length, 3); i++) {
              const colors = await getElementColors(page, `${selector}:nth-of-type(${i + 1})`);
              ...
              if (colors && colors.color && colors.backgroundColor) {
                ...
                if (ratio >= (isLarge ? WCAG_RATIOS.LARGE_TEXT : WCAG_RATIOS.NORMAL_TEXT)) {
                  summary.passedAA++;
                  
                  if (ratio >= (isLarge ? WCAG_RATIOS.ENHANCED_LARGE : WCAG_RATIOS.ENHANCED_TEXT)) {
                    summary.passedAAA++;
                  }
                } else {
                  summary.failed++;
                  summary.issues.push({
                    ...
                    required: isLarge ? WCAG_RATIOS.LARGE_TEXT : WCAG_RATIOS.NORMAL_TEXT
                  });
                }
              }
            }
    ...
    console.log(`Passed AA Standard: ${summary.passedAA} (${(summary.passedAA/summary.totalElementsTested*100).toFixed(1)}%)`);
    console.log(`Passed AAA Standard: ${summary.passedAAA} (${(summary.passedAAA/summary.totalElementsTested*100).toFixed(1)}%)`);
    console.log(`Failed: ${summary.failed} (${(summary.failed/summary.totalElementsTested*100).toFixed(1)}%)`);

    if (summary.issues.length > 0) {
      console.log('\n❌ CONTRAST ISSUES FOUND:');
      summary.issues.forEach(issue => {
        console.log(`  - ${issue.page} ${issue.element}: ${issue.ratio} (required: ${issue.required})`);
      });
    } else {
    ...
    expect(summary.passedAA / summary.totalElementsTested).toBeGreaterThanOrEqual(0.9);
  });
});