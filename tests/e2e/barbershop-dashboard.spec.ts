...
test.describe("Barbershop Dashboard", () => {
  ...
  test("should display dashboard statistics", async ({ page }) => {
    ...
    await expect(
      page.locator('[class*="text-2xl font-bold text-white"]')
    ).toHaveCount(4);
  });
  ...
  test("should navigate between dashboard tabs", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Gerencie a equipe de barbeiros")
    ).toBeVisible();

    await page.click("text=Serviços");
    await expect(
      page.locator("text=Gerencie os serviços oferecidos")
    ).toBeVisible();

    await page.click("text=Financeiro");
    await expect(
      page.locator("text=Acompanhe o desempenho financeiro")
    ).toBeVisible();
  ...
  test("should allow editing barbershop information", async ({ page }) => {
    ...
    const addressInput = page.locator(
      'input[value="Rua das Flores, 123 - Centro"]'
    );
  ...
  test("should show proper validation for empty barber form", async ({
    page,
  }) => {
    // Navigate to Barbeiros tab
    await page.click("text=Barbeiros");
  ...
  test("should show proper validation for empty service form", async ({
    page,
  }) => {
    // Navigate to Serviços tab
    await page.click("text=Serviços");
