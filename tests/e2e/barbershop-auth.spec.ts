...
test.describe('Barbershop Registration and Login', () => {
  ...
  test('should allow plan selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-plan="basic"]')).toHaveClass(/border-blue-500/);
    ...
    await expect(page.locator('[data-plan="enterprise"]')).toHaveClass(/border-purple-500/);
  });
  ...
  test('should validate required fields in step 1', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]')).toContainText('Preencha todos os campos obrigatórios');
  });
  ...
  test('should complete step 1 with valid data', async ({ page }) => {
    ...
    await expect(page.locator('text=Defina sua senha e finalize o cadastro')).toBeVisible();
  });
  ...
  test('should validate password requirements in step 2', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]')).toContainText('Senha deve ter pelo menos 6 caracteres');
  });
  ...
  test('should validate password confirmation', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]')).toContainText('Senhas não coincidem');
  });
  ...
  test('should require terms acceptance', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]')).toContainText('Você deve aceitar os termos de uso');
  });
  ...
  test('should allow going back to step 1', async ({ page }) => {
    ...
    await expect(page.locator('input[id="barbershopName"]')).toHaveValue('Barbearia Teste E2E');
  });
  ...
  test('should show error for invalid barbershop credentials', async ({ page }) => {
    await page.goto('/barbershop-login');
