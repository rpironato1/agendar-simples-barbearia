...
test.describe("Client Management System", () => {
  ...
  test("should display client management interface", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Dados consolidados dos clientes da barbearia")
    ).toBeVisible();
  });
  ...
  test("should display client table with correct headers", async ({ page }) => {
    ...
    await expect(
      page.locator('th:has-text("Taxa Cancelamento")')
    ).toBeVisible();
  ...
  test("should display cancellation rate levels correctly", async ({
    page,
  }) => {
    await page.click("text=Clientes");
    ...
    if (rowCount > 0) {
      ...
      const lowBadge = page.locator(
        '.bg-green-500\\/20.text-green-400:has-text("Baixo")'
      );
      const mediumBadge = page.locator(
        '.bg-yellow-500\\/20.text-yellow-400:has-text("Médio")'
      );
      const highBadge = page.locator(
        '.bg-red-500\\/20.text-red-400:has-text("Alto")'
      );

      const totalBadges =
        (await lowBadge.count()) +
        (await mediumBadge.count()) +
        (await highBadge.count());
  ...
  test("should show client appointment history in modal", async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      const appointmentCards = page.locator(
        '[data-testid="appointment-history"] .p-3'
      );
      ...
      if ((await appointmentCards.count()) > 0) {
        ...
        await expect(
          firstAppointment.locator("text=/\\d{2}\\/\\d{2}\\/\\d{4}/")
        ).toBeVisible(); // Date
        await expect(
          firstAppointment.locator("text=/R\\$\\s*\\d+/")
        ).toBeVisible(); // Price
      }
  ...
  test("should sort clients by total spent (descending)", async ({ page }) => {
    ...
    if (rowCount > 1) {
      ...
      for (let i = 0; i < Math.min(rowCount, 3); i++) {
        ...
        const spentValue = parseFloat(
          spentText?.replace(/[^\d,]/g, "").replace(",", ".") || "0"
        );
  ...
  test("should display correct client metrics calculation", async ({
    page,
  }) => {
    await page.click("text=Clientes");
    ...
    if (rowCount > 0) {
      ...
      const totalClientsText = await page
        .locator("text=Total de Clientes")
        .locator("..")
        .locator(".text-2xl")
        .textContent();
      ...
      const firstRow = clientRows.first();
      const appointmentsText = await firstRow
        .locator("td:nth-child(3)")
        .textContent();
      const cancelledText = await firstRow
