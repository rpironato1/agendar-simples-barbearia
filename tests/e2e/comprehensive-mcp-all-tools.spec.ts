...
test.describe("🧪 ALL MCP PLAYWRIGHT TOOLS - COMPREHENSIVE TESTING", () => {
  ...
  test("📱 Mobile-First Responsiveness - Landing Page", async ({
    page,
    isMobile,
  }) => {
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `landing-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
  ...
  test("🔐 Admin God Dashboard - Complete Flow", async ({ page, isMobile }) => {
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `admin-login-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `admin-dashboard-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    for (const tab of tabs) {
      ...
      if (await tabElement.isVisible()) {
        ...
        await page.screenshot({
          path: join(
            screenshotDir,
            `admin-${tab}-${isMobile ? "mobile" : "desktop"}.png`
          ),
          fullPage: true,
        });
      }
  ...
  test("🏢 Barbershop Dashboard - SaaS Features", async ({
    page,
    isMobile,
  }) => {
    // Login as barbershop
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `barbershop-dashboard-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    const planInfo = await page.evaluate(() => {
      const barbershop = JSON.parse(
        localStorage.getItem("current_barbershop") || "{}"
      );
      return {
    ...
    for (const section of sections) {
      ...
      if (await sectionLink.isVisible()) {
        ...
        await page.screenshot({
          path: join(
            screenshotDir,
            `barbershop-${section}-${isMobile ? "mobile" : "desktop"}.png`
          ),
          fullPage: true,
        });
      }
  ...
  test("📅 Calendar & Appointments - Interactive Testing", async ({
    page,
    isMobile,
  }) => {
    // Login as barbershop
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `calendar-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    if (eventCount > 0) {
      ...
      if (eventBox) {
        ...
        await page.screenshot({
          path: join(
            screenshotDir,
            `calendar-after-drag-${isMobile ? "mobile" : "desktop"}.png`
          ),
          fullPage: true,
        });
      }
    ...
    if (await newAppointmentBtn.isVisible()) {
      ...
      await page.screenshot({
        path: join(
          screenshotDir,
          `appointment-form-${isMobile ? "mobile" : "desktop"}.png`
        ),
        fullPage: true,
      });
    }
  ...
  test("💰 Financial POS System - Complete Flow", async ({
    page,
    isMobile,
  }) => {
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `financial-dashboard-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    if (await posButton.isVisible()) {
      ...
      await page.screenshot({
        path: join(
          screenshotDir,
          `pos-system-${isMobile ? "mobile" : "desktop"}.png`
        ),
        fullPage: true,
      });
      ...
      for (const method of paymentMethods) {
        ...
        if (await methodBtn.isVisible()) {
          ...
          await page.screenshot({
            path: join(
              screenshotDir,
              `payment-${method}-${isMobile ? "mobile" : "desktop"}.png`
            ),
            fullPage: true,
          });
        }
  ...
  test("👥 Client Management - CRUD Operations", async ({ page, isMobile }) => {
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `clients-list-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    if (await searchInput.isVisible()) {
      ...
      await page.screenshot({
        path: join(
          screenshotDir,
          `clients-search-${isMobile ? "mobile" : "desktop"}.png`
        ),
        fullPage: true,
      });
    }
    ...
    if (await newClientBtn.isVisible()) {
      ...
      await page.screenshot({
        path: join(
          screenshotDir,
          `new-client-form-${isMobile ? "mobile" : "desktop"}.png`
        ),
        fullPage: true,
      });
    }
  ...
  test("🔄 Subscription Plans - SaaS Testing", async ({ page, isMobile }) => {
    ...
    await page.screenshot({
      path: join(
        screenshotDir,
        `subscription-plans-${isMobile ? "mobile" : "desktop"}.png`
      ),
      fullPage: true,
    });
    ...
    for (const plan of plans) {
      ...
      if (await planCard.isVisible()) {
        ...
        await page.screenshot({
          path: join(
            screenshotDir,
            `plan-${plan}-hover-${isMobile ? "mobile" : "desktop"}.png`
          ),
          fullPage: true,
        });
        ...
        if (await selectButton.isVisible()) {
          ...
          await page.screenshot({
            path: join(
              screenshotDir,
              `plan-${plan}-selected-${isMobile ? "mobile" : "desktop"}.png`
            ),
            fullPage: true,
          });
  ...
  test("🛠️ Database System Verification", async ({ page }) => {
    ...
    const multienancyCheck = await page.evaluate(() => {
      const barbershops = JSON.parse(
        localStorage.getItem("barbershops") || "[]"
      );
      ...
      return {
        ...
        appointmentsWithBarbershopId: appointments.filter(
          (apt) => apt.barbershop_id
        ).length,
