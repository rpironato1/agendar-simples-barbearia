...
test.describe("Financial POS System", () => {
  ...
  test("should allow confirming payment for scheduled appointment", async ({
    page,
  }) => {
    ...
    if (await confirmButton.isVisible()) {
      ...
      await expect(
        page.locator("text=Cliente Chegou - Confirmar Pagamento")
      ).toBeVisible();
  ...
  test("should validate mixed payment total equals service price", async ({
    page,
  }) => {
    await page.click("text=Agenda");
    ...
    if (await confirmButton.isVisible()) {
      ...
      await expect(page.locator(".sonner-toast")).toContainText(
        "Valor total deve ser"
      );
    }
  });

  test("should update appointment status after payment confirmation", async ({
    page,
  }) => {
    await page.click("text=Agenda");
  ...
  test("should display payment method icon in payment status", async ({
    page,
  }) => {
    await page.click("text=Agenda");
    ...
    if ((await paidAppointments.count()) > 0) {
      ...
      await expect(page.locator('[data-testid="pix-icon"]')).toHaveCount({
        gte: 0,
      });
      await expect(page.locator('[data-testid="card-icon"]')).toHaveCount({
        gte: 0,
      });
      await expect(page.locator('[data-testid="cash-icon"]')).toHaveCount({
        gte: 0,
      });
    }
  });
  ...
  test("should show monthly revenue growth percentage", async ({ page }) => {
    ...
    const hasPositiveGrowth = await growthElement
      .locator("text=/\\+\\d+%/")
      .isVisible();
    const hasNegativeGrowth = await growthElement
      .locator("text=/-\\d+%/")
      .isVisible();
  ...
  test("should display total transactions count for today", async ({
    page,
  }) => {
    await page.click("text=Financeiro");
  ...
  test("should show financial summary cards with correct data", async ({
    page,
  }) => {
    await page.click("text=Financeiro");
    ...
    const summaryCards = page.locator(
      '[data-testid="financial-summary"] .text-2xl'
    );
