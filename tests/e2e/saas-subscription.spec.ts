...
test.describe('SaaS Subscription and Multitenancy', () => {
  test('should complete full barbershop registration and subscription flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-plan="premium"]')).toHaveClass(/border-amber-500/);
  ...
  test('should show different features based on subscription plan', async ({ page }) => {
  ...
  test('should demonstrate multitenancy - separate barbershop data', async ({ page }) => {
    ...
    await page.fill('input[id="barber-name"]', 'Barbeiro da Primeira Barbearia');
    ...
    await expect(page.locator('input[type="email"]')).toHaveValue('barbershop2@demo.com');
  });
  ...
  test('should enforce plan limits for Basic plan users', async ({ page }) => {
    ...
    await expect(page.locator('input[type="email"]')).toHaveValue('basic@demo.com');
  });

  test('should show trial period information for new signups', async ({ page }) => {
    await page.goto('/barbershop-signup');
  ...
  test('should display plan comparison on signup', async ({ page }) => {
    ...
    await expect(premiumPlan.locator('text=Barbeiros ilimitados')).toBeVisible();
    ...
    await expect(enterprisePlan.locator('text=Múltiplas filiais')).toBeVisible();
    await expect(enterprisePlan.locator('text=API personalizada')).toBeVisible();
  ...
  test('should show proper navigation between admin and barbershop dashboards', async ({ page }) => {
